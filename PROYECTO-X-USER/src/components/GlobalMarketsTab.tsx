...
// ─── Market Card ─────────────────────────────────────────────────────────────
function MarketCard({ market, pcts, outcomes, catCfg, geminixMkt, vol, tl, featured, onBetClick }: {
    market: PolyMarket;
    pcts: number[];
    outcomes: string[];
    catCfg: ReturnType<typeof getCatConfig>;
    geminixMkt?: GeminixMarket;
    vol: string;
    tl: string;
    featured?: boolean;
    onBetClick?: () => void;
        ...
        >
            ...
            <div style={{ display: 'flex', gap: 8 }}>
                <button
                    ...
                    onClick={geminixMkt && onBetClick ? onBetClick : undefined}
                    style={{
                        ...
                        cursor: geminixMkt ? 'pointer' : 'default',
            ...
            {geminixMkt && onBetClick && (
                <button onClick={onBetClick} style={{
                    ...
                    <Zap size={11} /> APOSTAR EN GEMINIX <ChevronRight size={11} />
                </button>
            )}
...
// ─── Main component ──────────────────────────────────────────────────────────
export default function GlobalMarketsTab({ user, walletBalance, onUpdateBalance, onSelectGeminixMarket }: {
    user: any;
    walletBalance: number;
    onUpdateBalance: () => void;
    onSelectGeminixMarket: (market: GeminixMarket) => void;
}) {
    const [markets, setMarkets] = useState<PolyMarket[]>([]);
    const [geminixMarkets, setGeminixMarkets] = useState<GeminixMarket[]>([]);
    ...
    const fetchGeminixMarkets = useCallback(async () => {
        const { data } = await supabase
            .from('prediction_markets')
            .select('id, polymarket_id, title, status, total_pool, options')
            .not('polymarket_id', 'is', null)
            .in('status', ['OPEN', 'CLOSED']);
        setGeminixMarkets(data || []);
    }, []);

    useEffect(() => {
        fetchPolymarket();
        fetchGeminixMarkets();
        const interval = setInterval(() => { fetchPolymarket(); fetchGeminixMarkets(); }, 120000);
        return () => clearInterval(interval);
    }, []);
    ...
    function getGeminixMarket(polyId: string) {
        return geminixMarkets.find(g => g.polymarket_id === polyId);
    }
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 24 }}>
                    {[
                        ...
                        { icon: <Zap size={18} />, val: geminixMarkets.length.toString(), label: 'En Geminix', color: '#00f3ff', grad: '#0ea5e9' },
                        ...
                        <div>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 16 }}>
                                {filtered.map((market, idx) => {
                                    ...
                                    const geminixMkt = getGeminixMarket(market.id);
                                    ...
                                    return (
                                        <div key={market.id} style={{ animation: `fadeInCard 0.4s ease-out ${idx * 30}ms both` }}>
                                            <MarketCard
                                                ...
                                                outcomes={outcomes}
                                                catCfg={catCfg}
                                                geminixMkt={geminixMkt}
                                                vol={vol}
                                                tl={tl}
                                                featured={idx === 0}
                                                onBetClick={geminixMkt ? () => onSelectGeminixMarket(geminixMkt) : undefined}
                                            />
                                        </div>
                                    );
                                })}
