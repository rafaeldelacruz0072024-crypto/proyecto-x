...
const PromoModal: React.FC<PromoModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 transition-all duration-500 ${isVisible ? 'bg-black/80 backdrop-blur-md opacity-100' : 'bg-transparent opacity-0 pointer-events-none'}`}>
      <div className={`relative max-w-md w-full bg-slate-950 rounded-2xl overflow-hidden shadow-2xl border border-geminix-accent/30 transition-all duration-500 transform ${isVisible ? 'scale-100 translate-y-0 opacity-100' : 'scale-95 translate-y-10 opacity-0'}`}>
        ...
        <div className="relative aspect-[4/5] w-full bg-slate-900 flex items-center justify-center overflow-hidden group">
          <img 
            ...
            alt="Geminix Special Edition Promo" 
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6 hidden [.fallback-promo_&]:flex bg-gradient-to-br from-purple-900/40 to-blue-900/40">
            <div className="w-16 h-16 rounded-full bg-geminix-accent/20 flex items-center justify-center mb-4">
              <svg className="w-8 h-8 text-geminix-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" /></svg>
            </div>
            <h3 className="text-2xl font-orbitron font-black text-white uppercase tracking-widest mb-2">GEMINIX<br/><span className="text-geminix-accent">SPECIAL EDITION</span></h3>
