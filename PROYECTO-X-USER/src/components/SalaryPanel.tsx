...
const SalaryPanel: React.FC<Props> = ({ teamVolume, onClaim, canClaim, dynamicSettings }) => {
  ...
  return (
    <div className="holo-card rounded-none clip-corner p-6 border-slate-800 bg-black/40 backdrop-blur-md relative overflow-hidden group">
      {/* HUD Accent Glow */}
      <div className={`absolute -right-4 -top-4 w-24 h-24 rounded-full blur-3xl opacity-10 transition-all duration-700 ${isSunday ? 'bg-geminix-green' : 'bg-geminix-gold'}`}></div>
      ...
      <div className="flex justify-between items-start mb-8 relative z-10">
        ...
        <div className={`px-4 py-1.5 rounded-full border transition-all duration-500 ${isSunday ? 'bg-geminix-green/10 border-geminix-green shadow-[0_0_10px_rgba(16,185,129,0.3)]' : 'bg-geminix-gold/10 border-geminix-gold/30'}`}>
          <span className={`text-[9px] font-black uppercase tracking-widest ${isSunday ? 'text-geminix-green animate-pulse' : 'text-geminix-gold opacity-60'}`}>
      ...
      {/* VENTANA DE PAGO */}
      <div className={`mb-6 border clip-corner p-4 flex flex-col md:flex-row items-center justify-between gap-3 transition-all duration-500 relative z-10 ${windowState.isOpen ? 'bg-geminix-green/10 border-geminix-green/30' : 'bg-slate-900/80 border-slate-700/50'}`}>
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 clip-corner flex items-center justify-center shrink-0 ${windowState.isOpen ? 'bg-geminix-green/20 text-geminix-green animate-pulse' : 'bg-slate-800 text-slate-400'}`}>
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          </div>
          <div>
            <p className={`text-[10px] font-black uppercase tracking-[0.2em] ${windowState.isOpen ? 'text-geminix-green' : 'text-slate-400'}`}>
        ...
        <div className={`px-3 py-2 border clip-corner ${windowState.isOpen ? 'bg-geminix-green/5 border-geminix-green/20' : 'bg-slate-950/80 border-slate-800'}`}>
      ...
      <div className="text-center mb-8 relative z-10">
        ...
        <div className="flex items-center justify-center gap-2">
          <span className="text-geminix-gold font-orbitron text-2xl font-bold">$</span>
        ...
        {isSunday && canClaim && currentTier.bonus > 0 && (
          <div className="flex items-center justify-center gap-2 mt-4">
            <div className="w-2 h-2 rounded-full bg-geminix-green animate-ping shadow-[0_0_10px_#10b981]"></div>
            <p className="text-[9px] text-geminix-green font-black uppercase tracking-widest">{t('salary.ready_to_claim')}</p>
          </div>
        )}
      </div>
      ...
      {teamVolume > 0 ? (
        <div className="space-y-4 relative z-10 mb-8 px-2">
          ...
          <div className="h-1.5 w-full bg-slate-900/80 rounded-none border border-slate-800 overflow-hidden relative">
            <div
              className="h-full bg-gradient-to-r from-geminix-gold/20 via-geminix-gold to-geminix-gold shadow-[0_0_15px_rgba(255,215,0,0.5)] transition-all duration-1000"
      ...
      <button
        ...
        className={`w-full py-4 clip-corner-sm font-orbitron font-black text-[10px] uppercase tracking-[0.3em] transition-all transform active:scale-95 flex items-center justify-center gap-3 ${canClaim && currentTier.bonus > 0 && isSunday
          ? 'bg-geminix-green text-slate-950 shadow-[0_0_20px_rgba(16,185,129,0.4)] hover:shadow-[0_0_30px_rgba(16,185,129,0.6)]'
      ...
      <div className="mt-6 pt-6 border-t border-slate-800/30 flex justify-between items-center relative z-10">
        ...
        <span className={`text-[9px] font-mono-tech uppercase tracking-widest ${isSunday ? 'text-geminix-green font-bold' : 'text-slate-500'}`}>
