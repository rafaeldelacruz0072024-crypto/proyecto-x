...
const DepositForm: React.FC<Props> = ({ onDeposit, userId, activePromotion, onClearPromo }) => {
  ...
  return (
    <div className="blue-glass rounded-2xl p-6 border border-slate-800 shadow-2xl animate-fade-in relative overflow-hidden">
      {/* Glow decorativo */}
      <div className="absolute -top-24 -right-24 w-48 h-48 bg-geminix-green/5 rounded-full blur-3xl pointer-events-none"></div>

      <div className="flex items-center mb-6">
        <div className="w-10 h-10 rounded-lg bg-geminix-green/10 flex items-center justify-center mr-3 border border-geminix-green/20">
          <svg className="w-6 h-6 text-geminix-green" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {activePromotion && !paymentData && (
        <div className="mb-6 animate-fade-in">
          <div className="bg-geminix-accent/10 border border-geminix-accent/30 rounded-xl p-4 relative group overflow-hidden">
            ...
            <div className="flex items-center justify-between gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-geminix-accent animate-pulse"></span>
                  <span className="text-[10px] font-black text-geminix-accent uppercase tracking-widest">Promoción Aplicada</span>
                </div>
                <h4 className="text-sm font-bold text-white tracking-tight">{activePromotion.title}</h4>
                <p className="text-[10px] text-slate-400 font-medium">Beneficio: <span className="text-geminix-gold font-bold">{activePromotion.type === 'CASHBACK' || activePromotion.type === 'DISCOUNT' ? `${activePromotion.reward_value}%` : `$${activePromotion.reward_value}`}</span></p>
        ...
        <form onSubmit={paymentMethod === 'card' ? handleStripePayment : handleGeneratePayment} className="space-y-6">
              ...
              <button
                ...
                className={`flex flex-col items-center gap-1.5 py-3 px-3 rounded-xl border text-[10px] font-black uppercase tracking-widest transition-all ${
                  paymentMethod === 'crypto'
                    ? 'bg-geminix-accent/10 border-geminix-accent text-geminix-accent'
          ...
          <div>
            ...
            <div className="relative">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-geminix-green font-black">$</span>
              <input
                type="number"
                value={amount}
                onChange={(e) => setAmount(e.target.value)}
                placeholder="0.00"
                className="w-full bg-slate-950 border border-slate-900 rounded-xl py-4 pl-8 pr-4 text-white text-lg font-black focus:outline-none focus:border-geminix-green transition-all placeholder:text-slate-800"
                required
          ...
          {paymentMethod === 'card' ? (
            ...
            <button
              ...
              className="w-full py-4 bg-geminix-green text-slate-950 font-black text-xs uppercase tracking-[0.3em] rounded-xl shadow-xl shadow-geminix-green/10 hover:brightness-110 active:scale-95 disabled:opacity-20 disabled:grayscale transition-all"
            >
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
          <div className="flex flex-col items-center">
            <div className="relative p-6 bg-white rounded-[2.5rem] shadow-[0_0_60px_rgba(0,243,255,0.15)] border-4 border-slate-900/10 mb-2">
              <div className="absolute -inset-1 bg-gradient-to-tr from-geminix-accent/20 to-geminix-green/20 rounded-[2.6rem] blur-xl opacity-50 -z-10 animate-pulse"></div>
              <img
            ...
            <div className="mt-4 flex flex-col items-center gap-2">
              ...
              <div className="flex items-center gap-2 bg-slate-950 border border-slate-900 rounded-full px-4 py-1.5">
                <span className="text-[10px] font-mono text-geminix-accent uppercase tracking-tighter">{paymentData.payment_id}</span>
                <button
                  onClick={() => {
            ...
            <div className="mt-6 flex flex-col items-center">
              <p className="text-[11px] text-geminix-green font-black uppercase tracking-widest mb-1">{t('deposit.send_exactly')}</p>
              <div className="flex items-center gap-3">
                <p className="text-3xl text-white font-black tracking-tighter text-glow-white">
                  {paymentData.pay_amount} <span className="text-geminix-accent">{paymentData.pay_currency.toUpperCase()}</span>
                </p>
                <button
                  onClick={() => {
          ...
          <div className="space-y-4 pt-2">
            <div>
              ...
              <div className="flex gap-2">
                <div className="flex-grow bg-slate-950 border border-slate-900 rounded-xl px-4 py-3.5 text-[10px] font-mono text-geminix-accent break-all flex items-center border-dashed relative group">
                  <div className="absolute inset-0 bg-geminix-accent/5 opacity-0 group-hover:opacity-100 transition-opacity rounded-xl"></div>
                  {paymentData.pay_address}
                </div>
                <button
                  onClick={handleCopy}
                  className={`p-3.5 rounded-xl transition-all shadow-xl ${isCopied ? 'bg-geminix-green text-slate-950' : 'bg-slate-900 text-slate-400 border border-slate-800 hover:border-geminix-accent hover:text-white'}`}
                >
                  {isCopied ? (
            ...
            <div className="bg-geminix-brand/5 p-5 rounded-2xl border border-geminix-brand/20 relative overflow-hidden group">
              <div className="absolute -right-4 -bottom-4 w-16 h-16 bg-geminix-brand/10 rounded-full blur-2xl group-hover:bg-geminix-brand/20 transition-all"></div>
              <p className="text-[10px] text-geminix-accent leading-relaxed text-center font-bold uppercase tracking-tight relative z-10">
            ...
            <div className="flex flex-col gap-3 pt-2">
              <button
                ...
                className="w-full py-4.5 bg-geminix-brand text-white font-black text-xs uppercase tracking-[0.3em] rounded-xl shadow-[0_10px_30px_rgba(0,114,255,0.3)] hover:brightness-110 active:scale-[0.98] transition-all font-orbitron border border-geminix-brand/50"
              >
