      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col h-[600px]">
        ...
        <div className="bg-slate-800 p-4 flex justify-between items-center border-b border-slate-700">
          <div className="flex items-center space-x-2">
            <svg className="w-5 h-5 text-geminix-gold" fill="currentColor" viewBox="0 0 20 20">
                ...
                <div className="p-6">
                  ...
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex gap-3">
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center border ${email.type === 'DEPOSIT' ? 'bg-geminix-green/10 border-geminix-green/20 text-geminix-green' : 'bg-geminix-gold/10 border-geminix-gold/20 text-geminix-gold'}`}>
                        {email.type === 'DEPOSIT' ? (
                      ...
                      <div>
                        <h4 className={`font-bold text-lg leading-tight ${email.type === 'DEPOSIT' ? 'text-geminix-green' : 'text-geminix-gold'}`}>{email.subject}</h4>
                    ...
                    {email.isConfirmed ? (
                      <span className="bg-geminix-green/10 text-geminix-green text-[9px] font-black px-2.5 py-1 rounded-full border border-geminix-green/20 uppercase tracking-tighter">{t('mock_email.status.confirmed')}</span>
                    ) : (
                      <span className="bg-geminix-gold/10 text-geminix-gold text-[9px] font-black px-2.5 py-1 rounded-full border border-geminix-gold/20 uppercase tracking-tighter animate-pulse">{t('mock_email.status.pending')}</span>
                    )}
                  </div>
                      ...
                      <>
                        ...
                        <div className="bg-slate-950/80 rounded-xl p-4 border border-slate-800 space-y-3">
                          ...
                          <div className="flex justify-between text-[10px]">
                            ...
                            <span className={`font-black ${email.isConfirmed ? 'text-geminix-green' : 'text-geminix-gold'}`}>
                        ...
                        {!email.isConfirmed ? (
                          <button
                            ...
                            className="w-full py-4 gold-gradient text-slate-900 font-black text-xs uppercase tracking-[0.2em] rounded-xl shadow-xl shadow-geminix-gold/20 hover:scale-[1.01] active:scale-[0.98] transition-all"
                          >
                      ...
                      <>
                        ...
                        <div className="bg-slate-950/80 rounded-xl p-4 border border-slate-800 space-y-3">
                          <div className="flex justify-between text-[10px]">
                            ...
                            <span className="text-geminix-green font-black">+${email.amount.toFixed(2)} USDT</span>
                          </div>
                          <div className="flex justify-between text-[10px]">
                          ...
                          <div className="flex justify-between text-[10px]">
                            ...
                            <span className={email.isConfirmed ? 'text-geminix-green font-black' : 'text-geminix-gold font-black'}>
                        ...
                        {!email.isConfirmed ? (
                          <button
                            onClick={() => onConfirm(email.txId, email.id)}
                            className="w-full py-4 blue-brand-gradient text-white font-black text-xs uppercase tracking-[0.2em] rounded-xl shadow-xl shadow-geminix-brand/20 hover:scale-[1.01] active:scale-[0.98] transition-all"
                          >
                            {t('mock_email.deposit.button')}
                          </button>
                        ) : (
                          <div className="bg-geminix-green/5 border border-geminix-green/10 rounded-xl p-4 text-center">
                            <p className="text-[10px] text-geminix-green font-black uppercase tracking-widest">
