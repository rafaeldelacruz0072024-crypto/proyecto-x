...
const TransactionHistory: React.FC<Props> = ({ transactions }) => {
  ...
  return (
    <div className="space-y-6">
      {/* Filter Bar */}
      <div className="holo-card p-4 rounded-none clip-corner border border-geminix-accent/20 flex flex-wrap gap-4 items-end">
        <div className="flex-1 min-w-[200px]">
          ...
          <select
            ...
            className="w-full bg-black/50 border border-slate-800 clip-corner-sm py-2 px-3 text-sm text-white font-mono-tech focus:outline-none focus:border-geminix-accent transition-all"
          >
        ...
        <div>
          ...
          <input
            ...
            className="bg-black/50 border border-slate-800 clip-corner-sm py-2 px-3 text-sm text-slate-300 font-mono-tech focus:outline-none focus:border-geminix-accent transition-all uppercase"
          />
        </div>
      ...
      {/* Results Table */}
      <div className="holo-card p-0 rounded-none clip-corner overflow-hidden shadow-[0_0_40px_rgba(0,0,0,0.5)] border border-geminix-accent/20 relative group/table">
        {/* Horizontal Scanner */}
        <div className="absolute top-[52px] left-0 w-full h-[1px] bg-geminix-accent/40 shadow-[0_0_10px_cyan] animate-scanline-h z-30 pointer-events-none"></div>

        <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl from-geminix-accent/10 to-transparent pointer-events-none"></div>
                ...
                paginatedData.map((tx) => {
                  ...
                  return (
                    <tr
                      ...
                      className="hover:bg-geminix-accent/[0.03] transition-all group cursor-pointer relative"
                    >
                      <td className="px-6 py-6 font-mono-tech text-[10px] text-slate-500 group-hover:text-geminix-accent transition-colors tracking-widest">
                      ...
                      <td className="px-6 py-6 max-w-xs border-l border-white/[0.02]">
                        <div className="flex items-center gap-2 mb-2">
                          <span className={`w-1.5 h-1.5 rounded-none rotate-45 ${tx.amount > 0 ? 'bg-geminix-green shadow-[0_0_5px_rgba(16,185,129,1)]' : 'bg-red-500 shadow-[0_0_5px_rgba(239,68,68,1)]'}`}></span>
                          <span className={`text-[9px] font-orbitron font-black tracking-[0.1em] uppercase ${tx.amount > 0 ? 'text-geminix-green' : 'text-red-400'}`}>
                      ...
                      <td className="px-6 py-6 border-l border-white/[0.02]">
                        <div className={`inline-flex items-center px-3 py-1 rounded-none border text-[9px] font-orbitron font-black uppercase tracking-widest ${
                          tx.status === TransactionStatus.COMPLETED || tx.status === TransactionStatus.CONFIRMED || tx.status === 'COMPLETED' || tx.status === 'CONFIRMED'
                            ? 'bg-geminix-green/10 border-geminix-green/30 text-geminix-green'
                            : tx.status === TransactionStatus.REJECTED || tx.status === TransactionStatus.FAILED || tx.status === 'REJECTED' || tx.status === 'FAILED' || tx.type === 'WITHDRAWAL_REJECTED'
                              ...
                              : 'bg-geminix-gold/10 border-geminix-gold/30 text-geminix-gold animate-pulse'
                        }`}>
                          {tx.type === 'WITHDRAWAL_REJECTED' ? 'DEVUELTO' : tx.status}
                        </div>
                      </td>
                      <td className={`px-6 py-6 text-right font-orbitron font-black text-xl border-l border-white/[0.02] ${tx.amount > 0 ? 'text-geminix-green text-glow-green' : 'text-red-500 text-glow-red'}`}>
                ...
                {[...Array(totalPages)].map((_, i) => (
                  <button
                    ...
                    className={`w-8 h-8 clip-corner-sm text-[10px] font-orbitron font-bold transition flex items-center justify-center ${currentPage === i + 1
                      ? 'bg-geminix-accent text-black shadow-[0_0_10px_cyan]'
      ...
      {selectedTx && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/90 backdrop-blur-xl animate-fade-in">
          <div className="holo-card p-0 rounded-none clip-corner border border-geminix-accent rounded-[0] w-full max-w-lg overflow-hidden shadow-[0_0_50px_rgba(0,243,255,0.2)]">
            <div className="p-8 relative z-10">
              <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-geminix-accent to-transparent"></div>

              {/* Header */}
              <div className="flex justify-between items-start mb-8">
                <div>
                  <h3 className="text-[10px] font-mono-tech font-bold text-geminix-accent uppercase tracking-[0.3em] mb-1">{t('finance.audit_protocol')}</h3>
                ...
                <button
                  ...
                  className="w-10 h-10 clip-corner-sm bg-slate-900 border border-slate-800 flex items-center justify-center text-slate-500 hover:text-white hover:border-geminix-accent transition-colors"
                >
              ...
              <div className="bg-black/50 rounded-none clip-corner-sm p-8 text-center border border-slate-900 mb-8 relative overflow-hidden group">
                <div className="absolute top-0 left-0 w-full h-1 bg-geminix-brand opacity-40"></div>
                <p className="text-[10px] font-mono-tech font-bold text-slate-500 uppercase tracking-widest mb-2">{t('finance.tx_volume')}</p>
                <div className={`text-4xl font-orbitron font-bold ${selectedTx.amount > 0 ? 'text-geminix-green text-glow-green' : 'text-red-500 text-glow-red'}`}>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-900/40 p-4 clip-corner-sm border border-slate-800/50">
                    ...
                    <div className="flex items-center gap-2">
                      <div className={`w-2 h-2 rotate-45 ${selectedTx.status === 'COMPLETED' || selectedTx.status === 'CONFIRMED' ? 'bg-geminix-green shadow-[0_0_5px_green]' : 'bg-geminix-gold animate-pulse shadow-[0_0_5px_gold]'}`}></div>
                ...
                <div className="space-y-3 pt-2 border-t border-slate-800/50 mt-4">
                  <div className="flex justify-between items-center text-[10px] px-2 pt-2">
                    ...
                    <span className="text-geminix-accent font-mono font-bold text-[9px]">{String(selectedTx.id).slice(0, 20)}...</span>
                  </div>
              ...
              <div className="mt-8 flex gap-3">
                <button
                  ...
                  className="flex-grow py-4 bg-slate-900 border border-slate-800 clip-corner-sm text-[10px] font-orbitron font-bold text-slate-400 uppercase tracking-widest hover:text-white hover:border-geminix-accent transition-all flex items-center justify-center gap-2"
                >
                ...
                <button
                  onClick={() => setSelectedTx(null)}
                  className="px-10 py-4 bg-geminix-brand text-white clip-corner-sm text-[10px] font-orbitron font-bold uppercase tracking-widest shadow-[0_0_20px_rgba(0,114,255,0.3)] active:scale-95 transition-all hover:brightness-110"
                >
