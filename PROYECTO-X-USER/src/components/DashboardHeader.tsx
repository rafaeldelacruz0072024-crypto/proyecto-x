...
const DashboardHeader: React.FC<Props> = ({ user, balance, onProfileClick, onMenuToggle }) => {
  ...
  return (
    <header className="flex flex-col md:flex-row justify-between items-center p-3.5 sm:p-6 bg-black/60 backdrop-blur-xl border-b border-geminix-accent/30 sticky top-0 z-50 shadow-[0_4px_30px_rgba(0,243,255,0.1)]">

      <div className="flex items-center space-x-4 relative z-10">
        {/* HAMBURGER — mobile only */}
        <button
          onClick={onMenuToggle}
          className="flex items-center gap-2 px-3 h-9 border border-geminix-accent/30 bg-black/40 hover:bg-geminix-brand/20 transition-all"
          aria-label="Open menu"
        >
          <div className="flex flex-col justify-center gap-1.5">
            <span className="w-5 h-[1.5px] bg-geminix-accent"></span>
            <span className="w-5 h-[1.5px] bg-geminix-accent"></span>
            <span className="w-3 h-[1.5px] bg-geminix-accent self-start"></span>
          </div>
          <span className="text-[9px] font-orbitron font-black text-geminix-accent uppercase tracking-[0.2em] md:hidden">MENU</span>
        </button>

        {/* LOGO */}
        <div className="relative group cursor-pointer" onClick={onProfileClick}>
          <div className="absolute inset-0 bg-geminix-accent blur-2xl opacity-10 group-hover:opacity-30 transition-opacity"></div>
        ...
        <div className="hidden md:flex flex-col border-l border-white/10 pl-4">
          <p className="text-[10px] uppercase text-geminix-accent font-orbitron font-bold tracking-[0.3em] text-glow-cyan">
            GEMINIX OS V1.1 (PATCHED)
          </p>
      ...
      <div className="mt-4 md:mt-0 flex items-center space-x-8 relative z-10">
        <div className="text-right hidden md:block">
          ...
          <div className="flex items-center justify-end gap-2 group">
            <p className="text-2xl font-orbitron font-bold text-white group-hover:text-glow-white transition-all">
              <span className="text-geminix-green mr-1">$</span>
        ...
        <div className="flex items-center gap-4">
          {/* AVATAR */}
          <div className="relative group cursor-pointer" onClick={onProfileClick}>
            <div className="absolute -inset-1 bg-gradient-to-r from-geminix-accent to-geminix-brand rounded-full blur opacity-20 group-hover:opacity-75 transition duration-500"></div>
            <div className="relative h-10 w-10 text-sm clip-corner-sm bg-black border border-geminix-accent/50 flex items-center justify-center text-white font-orbitron font-bold shadow-lg">
              {userInitial}
            </div>

            {/* STATUS BADGE */}
            <div className="absolute -bottom-1 -right-1 h-3 w-3 bg-black rounded-full flex items-center justify-center">
              <div className="h-2 w-2 bg-geminix-green rounded-full animate-pulse"></div>
            </div>
          </div>

          <div className="md:hidden text-left">
            <p className="text-[10px] text-white font-orbitron uppercase">{userName}</p>
            <p className="text-[8px] text-geminix-accent font-mono-tech">ID: #{user?.id?.slice(0, 4)}</p>
          </div>
        </div>
