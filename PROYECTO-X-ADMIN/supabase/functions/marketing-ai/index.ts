...
const STAGE_CONTEXT: Record<string, string> = {
  ...
  interes: 'ETAPA DE INTERÉS: El objetivo es EDUCAR sobre la oportunidad. Explica brevemente qué es NOVA DIGITAL (plataforma de finanzas digitales, mercados de predicción, inversiones cripto y red de afiliados). Muestra el potencial sin presionar.',
  ...
  accion: 'ETAPA DE ACCIÓN: El objetivo es generar CONVERSIÓN. CTA directo y urgente para registrarse en NOVA DIGITAL usando el link de referido del usuario. Crea urgencia legítima (plazas limitadas, oferta de tiempo).',
...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `Eres un experto en marketing digital y network marketing para NOVA DIGITAL, una plataforma de finanzas digitales que incluye:
...
Tu misión: crear contenido de marketing VIRAL y AUTÉNTICO que ayude a los miembros de NOVA DIGITAL a crecer su red de referidos en redes sociales.
