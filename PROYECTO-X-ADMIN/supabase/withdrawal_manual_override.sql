-- ============================================================
-- MANUAL OVERRIDE: Desbloqueo temporal de ventana de retiros
-- Agrega columna withdrawal_override_until a system_settings
-- y actualiza el RPC para respetarla.
-- ============================================================

-- 1. Agregar columna de override
ALTER TABLE public.system_settings
  ADD COLUMN IF NOT EXISTS withdrawal_override_until TIMESTAMPTZ DEFAULT NULL;

-- 2. Actualizar RPC create_withdrawal_request para respetar el override
...
BEGIN
  ...
  IF NOT v_is_override_active AND NOT p_bypass_window AND v_settings.withdrawal_window_enabled = TRUE THEN
    ...
    v_current_day  := EXTRACT(DOW FROM v_current_time); -- 0=domingo

    -- Verificar día (si está configurado)
  ...
  -- 7. Validar límite diario (máx 2 retiros por día)
  ...
  IF v_daily_count >= 2 THEN
    RETURN jsonb_build_object('success', false, 'error', 'Has alcanzado el límite de 2 retiros por día.');
  END IF;
