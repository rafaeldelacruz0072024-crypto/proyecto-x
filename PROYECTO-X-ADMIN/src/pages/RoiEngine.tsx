...
const RoiEngine: React.FC = () => {
  ...
  const load = async () => {
    ...
    const [investmentResult, transactionResult] = await Promise.all([
      supabase.from('investments').select('id, amount, assigned_roi_percentage, accumulated_earnings, business_days_elapsed, matures_on, status, plans:plan_id(name, code, duration_business_days, payout_mode)').order('created_at', { ascending: false }),
      supabase.from('transactions').select('id, type, amount, description, created_at').in('type', ['DAILY_RETURN', 'MATURITY_PAYOUT']).order('created_at', { ascending: false }).limit(50),
    ]);
      ...
      <header className="flex flex-wrap items-center justify-between gap-6">
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <p className="mt-2 text-xs font-bold uppercase tracking-[0.25em] text-slate-500">Tasas fijas por contrato · días hábiles · liquidación segura</p>
          </div>
        </div>
      ...
      <section className="overflow-hidden rounded-[2rem] border border-white/5 bg-[#121217]">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="bg-black/20 text-[10px] uppercase tracking-widest text-slate-500"><tr><th className="p-5">Plan</th><th className="p-5">Capital</th><th className="p-5">Tasa</th><th className="p-5">Días</th><th className="p-5">Ganancia</th><th className="p-5">Estado</th></tr></thead>
            <tbody className="divide-y divide-white/5">
              {loading ? <tr><td colSpan={6} className="p-16 text-center"><Loader2 className="mx-auto animate-spin text-indigo-400" /></td></tr> : investments.map(item => (
                <tr key={item.id} className="text-slate-300"><td className="p-5 font-bold text-white">{item.plans?.name || 'Plan'}</td><td className="p-5">${Number(item.amount).toFixed(2)}</td><td className="p-5 text-indigo-300">{Number(item.assigned_roi_percentage).toFixed(3)}%</td><td className="p-5">{item.business_days_elapsed}{item.plans?.duration_business_days ? `/${item.plans.duration_business_days}` : ''}</td><td className="p-5 text-emerald-400">${Number(item.accumulated_earnings).toFixed(2)}</td><td className="p-5">{item.status}</td></tr>
              ))}
            </tbody>
          </table>
        </div>
