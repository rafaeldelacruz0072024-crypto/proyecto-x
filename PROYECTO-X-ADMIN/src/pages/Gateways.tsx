...
const Gateways: React.FC = () => {
  ...
  const validateKeys = (provider: string, apiKey: string, secretKey?: string, webhookSecret?: string): string | null => {
    if (provider.trim().toUpperCase() !== 'CRYPTOP') return 'NOVA solo admite la pasarela CRYPTOP.';
    if (apiKey.trim().length < 8) return 'La credencial de CRYPTOP parece demasiado corta.';
    if (apiKey.includes(' ')) return 'La credencial de CRYPTOP no puede contener espacios.';
    if (secretKey?.includes(' ') || webhookSecret?.includes(' ')) return 'Las credenciales no pueden contener espacios.';
    return null;
  };
  ...
  const handleCreateGateway = async (e: React.FormEvent) => {
    ...
    const validationError = validateKeys('CRYPTOP', newGatewayData.api_key, newGatewayData.secret_key, newGatewayData.webhook_secret);
    if (validationError) {
  ...
  const handleUpdateGateway = async (gateway: GatewayConfig) => {
    ...
    const validationError = validateKeys('CRYPTOP', gateway.api_key, gateway.secret_key, gateway.webhook_secret);
    if (validationError) {
      setError(validationError);
      return;
    }
          ...
          <div key={gw.id} className={`group bg-[#111114] border-2 ${gw.is_active ? 'border-yellow-500/20' : 'border-slate-800/50'} rounded-[3.5rem] overflow-hidden shadow-2xl transition-all duration-500 hover:translate-y-[-4px] hover:border-yellow-500/40`}>
            ...
            <div className="p-10 border-b border-slate-800/50 flex flex-col md:flex-row md:items-center justify-between gap-8 bg-gradient-to-r from-transparent to-white/[0.01]">
              <div className="flex items-center space-x-6">
                ...
                <div>
                  <h3 className="text-3xl font-black text-white tracking-tighter uppercase italic">CRYPTOP</h3>
                  <div className="flex items-center gap-3 mt-2">
                    ...
                    <span className="text-[9px] text-slate-500 font-bold uppercase tracking-[0.2em]">BEP20 BNB Â· Sincronizado: {new Date(gw.updated_at).toLocaleDateString()}</span>
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="relative group/url">
                    <input
                      type="text"
                      readOnly
                      value={'Pendiente: configurar webhook oficial CRYPTOP en Supabase'}
              ...
              <div className="space-y-8">
                <div className="space-y-4">
                  ...
                  <input
                    ...
                    placeholder="Credencial server-side de CRYPTOP"
                  />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  <div className="space-y-4">
                    ...
                    <input
                      ...
                      placeholder="Secreto server-side de CRYPTOP"
                    />
                  </div>
                  <div className="space-y-4">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] px-2 flex items-center gap-2">
                    ...
                    <input
                      ...
                      placeholder="Secreto de webhook de CRYPTOP"
                    />
                  </div>
                </div>
              </div>
